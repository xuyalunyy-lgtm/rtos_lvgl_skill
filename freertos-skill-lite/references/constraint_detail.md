...
| ID | 约束 | 严重度 | 验证 | 正例 | 反例 |
|----|------|--------|------|------|------|
| C18.1 | GPIO 方向必须在使用前配置（gpio_config 先于 gpio_set_level） | P0 | 人工 + grep | `gpio_config()` → `gpio_set_level()` | 未 config 直接 set_level |
| C18.2 | I2C 设备地址必须来自 datasheet，禁止硬编码猜测 | P1 | 人工 | `#define OLED_ADDR 0x3C /* datasheet */` | `#define OLED_ADDR 0x60` 无依据 |
...
| ID | 约束 | 严重度 | 验证 | 正例 | 反例 |
|----|------|--------|------|------|------|
| C19.1 | NVS 写入后必须 `nvs_commit()` + 检查返回值 | P0 | 人工 | `nvs_set_str` → `nvs_commit` → `nvs_close` | 不 commit 不检查返回值 |
...
| ID | 约束 | 严重度 | 验证 | 正例 | 反例 |
|----|------|--------|------|------|------|
| C20.1 | WiFi/WSS 断线重连必须有指数退避（1s→2s→…→60s cap） | P0 | 人工 | `WIFI_RECONNECT_BASE_MS * (1 << retry)` | 立即 `esp_wifi_connect()` tight loop |
| C20.2 | 所有阻塞网络操作（recv/send/connect）必须有有限超时 | P0 | 人工 + grep | `setsockopt(SO_RCVTIMEO, 10s)` | `recv(sock, buf, len, 0)` 无超时 |
...
| ID | 约束 | 严重度 | 验证 | 正例 | 反例 |
|----|------|--------|------|------|------|
| C21.1 | 深度睡眠前必须保存运行状态到 NVS/Flash（`nvs_commit` + 检查返回值） | P0 | 人工 | `nvs_set_u8` → `nvs_commit` → `esp_deep_sleep_start` | 直接 `esp_deep_sleep_start` 丢失状态 |
...
| C21.4 | 深度睡眠前必须逐个关闭外设电源（LCD 背光/音频 DAC/WiFi/PSRAM） | P1 | 人工 | `power_down_peripherals()` 逐个关闭 | 仅 `esp_deep_sleep_start` 不关外设 |
...
| C23.5 | 帧缓冲大小必须根据 RAM 可用性选择：PSRAM 可用→全屏双缓冲；RAM 不足→部分刷新（1/5 或 1/10 屏）；分配失败必须检查 | P0 | 人工 | `heap_caps_malloc` 返回值检查 | 未检查 `malloc` 返回 |
| C23.6 | `lv_disp_drv_t` 注册必须设置 `hor_res`、`ver_res`、`draw_buf`、`flush_cb`；禁止遗漏必要字段 | P1 | 人工 | 完整 `lv_disp_drv_init` + 字段赋值 | 缺少 `hor_res`/`ver_res` |
