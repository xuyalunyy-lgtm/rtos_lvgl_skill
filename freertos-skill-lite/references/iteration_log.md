...
- **平台：** 通用
- **变更：**
  - `network_resilience_checker.py`：C20.2 超时检查从空操作改为实际检测；C20.1 退避状态机改为函数级花括号计数；recv/send/connect 使用词边界正则
  - `blocking_wait_checker.py`：移除 xSemaphoreCreateMutex/xSemaphoreCreateBinary；改用词边界正则；函数上下文检测扩展
  - `display_driver_checker.py`：C23.6 补充 draw_buf 缺失报告
  - `peripheral_driver_checker.py`：C18.1 添加 gpio_set_direction
  - `low_power_checker.py`：C21.4 POWER_DOWN_INDICATORS 收窄
  - `flash_nvs_checker.py`：C19.1 添加 ESP_ERROR_CHECK 宏识别
- **验证：** 待 CI
- **版本：** 4.6.1
