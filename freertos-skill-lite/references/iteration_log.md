...
### 2026-07-02 — v23.0.8 Log-First Bug Triage System

- **来源：** V23 路线图
- **平台：** 通用
- **变更：**
  - `tools/log_triage.py` — 日志快速定位 bug（12 种症状路由，7 项自测）
  - `references/log_symptom_routes.json` — 症状路由表
  - `tools/fixtures/logs/` — 6 个 bad log + 1 个 good log 样本
  - `scripts/check_log_triage_matrix.py` — 日志样本回归矩阵（6/6 PASS）
- **验证：** log_triage 7/7 / check_log_triage_matrix 6/6 / skill_iterate 12/12
- **版本：** 23.0.8
