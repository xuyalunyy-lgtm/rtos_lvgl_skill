...
> 27 个约束域、149 条规则之间的关系网络。修改一条约束时，参考本图评估连锁影响。
...
| C27 → C7 | jitter buffer 深度、补静音和 resync 状态直接影响 RAM 与固定块策略 |
| C4+C23+C25+C26 → C28 | 媒体 DMA/cache buffer 依赖 DMA 安全、显示 flush、帧池队列和媒体格式大小 |
| C28 → C7 | DMA-capable pool、cache line 对齐和零拷贝 frame pool 直接影响 RAM/PSRAM 分配策略 |
...
| **C25.3** 有界 video queue | **C7.4** 高频路径固定块 | 视频不丢帧 vs RAM 上限 | 预分配短 ring，满时 drop-oldest，禁止扩容阻塞 |
...
| **C28.1** DMA-capable 内存 | **C7.4** 固定块内存优化 | PSRAM 容量 vs DMA 可访问性 | DMA 热路径用 internal/DMA SRAM；PSRAM 仅放冷数据或显式同步 copy |
