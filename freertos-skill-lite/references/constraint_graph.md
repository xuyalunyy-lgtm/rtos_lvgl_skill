...
> 27 个约束域、150 条规则之间的关系网络。修改一条约束时，参考本图评估连锁影响。
...
| C28 → C7 | DMA-capable pool、cache line 对齐和零拷贝 frame pool 直接影响 C7.8/C7.10 的 RAM/PSRAM 分配策略 |
...
| 改 C7（内存优化） | C6（SDK 裁剪）、C19（Flash/NVS）、C28（DMA/cache buffer） | 裁剪释放内存；外部 RAM 优先策略须避开 DMA/实时路径 |
