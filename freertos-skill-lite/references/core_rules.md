...
| 层 | Android 对标 | 职责 |
|----|-------------|------|
| Model | Background Service / 网络回调 | 采集/网络；`xQueueSend` [QUEUE_SEND] = `sendMessage` |
| Presenter | `Handler.handleMessage` | Looper 消费 Queue；**释放 payload** |
| View | `runOnUiThread` | 仅 LVGL；`lv_async_call` [ASYNC_CALL] 刷新 |
...
| # | 主题 | 细则 | 子约束数 |
|---|------|------|----------|
| 1 | LVGL 线程安全 | 后台禁止 `lv_obj_*` [OBJ_CREATE/OBJ_SET]；`lv_async_call` [ASYNC_CALL] 或 mutex → [lvgl_thread_safety.txt](../prompts/lvgl_thread_safety.txt) | 7 |
...
| 4 | 音频 DMA | ISR 仅 `*FromISR` [SEM_GIVE/QUEUE_SEND (ISR variant)]；Cache 一致性 → [audio_dma_pingpong.txt](../prompts/audio_dma_pingpong.txt) | 8 |
...
| 正例 时钟/Jitter | 完整版 `examples/good_av_clock_jitter.c` |
| 反例 时钟/Jitter | 完整版 `examples/bad_av_clock_jitter.c` |
| 正例 DMA/cache buffer | 完整版 `examples/good_av_dma_buffer_lifecycle.c` |
| 反例 DMA/cache buffer | 完整版 `examples/bad_av_dma_buffer_lifecycle.c` |
