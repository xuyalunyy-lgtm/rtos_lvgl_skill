...
## JL LVGL + 视频/页面切换现场检查清单

当 JL/AC79/AC792/WL83 产品同时包含 LVGL 页面、TF 卡 UI 资源、触摸切页、本地音视频播放时，优先按这张清单排查。

### 1. 先拆开验证资源与解码器

- 先确认 TF/SD 已挂载、运行时路径完全正确，再改布局代码。背景能显示但图标不显示，通常说明文件系统通了，但某类资源的解码器、路径前缀或资源层仍有问题。
- 不要因为 LVGL 有 image 控件就假设 PNG 已可用。必须确认 `lv_conf.h` 或本地资源层选择的 PNG decoder 已经参与链接。类似 `lodepng_decode32` undefined reference 的错误，表示声明/调用存在，但实现没有链接进来。
- 全屏背景优先用 JPEG/SJPG；小透明图标可用 PNG/RAW/QOI。返回、电池、收藏、播放、WiFi 成功/失败等复用图标，应启动后或首次使用时解码一次并缓存 descriptor。
- 设计图整页图片只能作为临时对照，不应作为正式页面实现。正式页面要由 BG + ICON + 文本/组件组合，便于复用和后续交互。

### 2. 页面切换性能规则

- 触摸事件里不要同步解码大图、扫描目录或重建所有共享资源。图片访问统一走公共资源层和缓存。
- 内存允许时保留当前页和前/后一页对象。每次点击都销毁并重建整页，会造成明显卡顿并增加碎片风险。
- 页面切换期间屏蔽输入。小 RTOS UI 栈上，连续点击容易让对象删除和创建交错，表现为旧标题残留、组件叠加或触摸失效。
- 所有外部 UI 更新必须走 LVGL/UI 任务：`lv_async_call`、UI RPC 或项目内 view queue。媒体、网络、音频回调禁止直接调用 `lv_obj_*`。

### 3. 视频覆盖播放生命周期

- 短视频播放时保持 LVGL 和主 framebuffer (`fb0`) 活着，不要围绕播放去 close/reopen `fb0`。
- 视频放到专用顶层 display layer/framebuffer，例如 `fb4`，并确认 display combine 路径把它放在 LVGL 上层。
- 播放结束后由媒体任务请求 UI 任务 reload 或导航；媒体任务不直接重建 LVGL 页面。
- 健康顺序应该是：媒体报告播放结束 -> UI RPC 执行 reload/nav -> UI 完成回调清除 video/page-switch busy 状态。

### 4. 必须补齐尾段状态机

“播放结束 -> 页面切换完成”必须覆盖完整状态，而不是只用一个 playing bool：

```c
enum {
    VIDEO_IDLE = 0,
    VIDEO_PLAYING,
    VIDEO_UI_NAV_PENDING,
};
```

规则：

- `IDLE -> PLAYING`：只有接受新的播放请求时进入。
- `PLAYING -> UI_NAV_PENDING`：媒体 loop 返回后、投递 UI reload/nav 前进入。
- `UI_NAV_PENDING -> IDLE`：只能在 UI 线程完成回调里，等 `ui_main_reset_to_next_page()` 或 page reload 返回后进入。
- 状态非 `IDLE` 时，触摸、返回、重复播放请求全部忽略。
- 必须保留两个日志边界：`video done, nav next` 和 `video ui nav done`。如果只有前者没有后者，问题在 UI RPC/navigation，不在媒体解码。

### 5. 日志快速判定

| 日志/现象 | 高概率原因 | 第一检查点 |
|---|---|---|
| PNG 文件存在但图标空白 | decoder 未链接、格式不支持、路径前缀错误 | `lv_conf.h`、资源层、link map |
| 背景显示但切页 FPS 掉到 1-2 | 触摸路径同步解码或整页重建 | descriptor 缓存、event cb 禁止阻塞 |
| 视频能播，结束后 `fb0` open/reopen 失败 | LVGL framebuffer 生命周期竞态 | 保持 `fb0` 活着，使用顶层视频 layer |
| 视频后页面残留旧标题/叠加 | 上一次 rebuild 未完成就被重入 | transition busy/state machine |
| 播放后触摸失效 | 输入屏蔽未释放或 busy 状态未清 | 查 UI completion callback 日志 |


## JL AC792/WL83 LVGL + TF assets + media playback field rules (2026-07-07)

Use this section when the target project is AC792/WL83 and combines LVGL pages, TF-card image assets, touch page switching, and local audio/video playback.

- Treat TF mount, image decoder, and LVGL display as separate layers. A mounted TF card does not prove PNG support; a visible JPEG background does not prove icons are decoded.
- Confirm image decoder build/link before layout work. For PNG, check `LV_USE_PNG` or the project-specific decoder implementation and final link symbols.
- Background JPEGs are expensive on 480x800 screens. If logs show repeated `jpeg_dec0_task` during taps, prioritize common background cache, descriptor reuse, and page reuse before tuning coordinates.
- Keep `fb0`/LVGL alive during short video overlay playback. Use a top display layer/framebuffer for video and return to LVGL via UI-task navigation.
- Audio/video completion must not directly manipulate LVGL objects. Media callbacks post UI work; UI completion clears `*_UI_NAV_PENDING`.
- Ignore touch/back/restart while page transition or media tail navigation is pending. This avoids duplicate page creation, stale title overlays, and next-page misrouting.
- For task sizing on JL, remember `stack_size` is in words, not bytes. Expanding LVGL/media related stacks can mask overflow symptoms, but it does not fix lifecycle races.
