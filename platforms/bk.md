...
- [带屏语音产品通用模式](#带屏语音产品通用模式bk-avdk-ap-类)
- [WSS 与外设收尾通用模式](#wss-与外设收尾通用模式bk-avdk-ap-类)
- [与其他平台的差异](#与其他平台的差异)
...
## 带屏语音产品通用模式（BK AVDK AP 类）

以下模式来自 BK7258 AVDK AP 侧现场 review，已抽象为通用带屏语音/媒体产品重构参考。
...
- 关键事件（业务会话/网络/OTA/深睡）队列满时可短重试；深度建议 ≥32
...
## WSS 与外设收尾通用模式（BK AVDK AP 类）

来源：BK7258 AVDK AP 侧 WSS、外设收尾、资源加载和裁剪闭环；以下规则已抽象为通用模式。

### 密钥（C9）

- 非 AVDK `ap/` 工程：`projects/<app>/config/bk7258/config` + `config.secrets` + `scripts/merge_config_secrets.sh`
- 云 API 密钥用 `CONFIG_APP_CLOUD_*`（Kconfig.projbuild），禁止在业务 `.c` 文件里硬编码 `#define`
...
**勿裁：** `wn_module_cgi.c`（SoftAP 配网）、`iot_volume.c`（须在 `iot_devices_init` 注册服务处理器，勿依赖已移除的 `http_client` init）。
...
| 任务 | 建议栈 (bytes) |
|------|----------------|
| `asset_render_task` / 外设数据任务 | 8192–24576，按资源解码/外设负载实测 |
| `vc_start`（voicechat 异步建链） | ≥5120 |
| WSS / dialog / session manager | ≥4096，须 HighWaterMark 实测 |
...
### littlefs UI 资源（LVGL decode fail:0）

UI 读 littlefs/flash 中的 PNG、QOI、RAW 等资源。`lv_png_img_load` 返回失败时 LVGL8 常打 `decode fail:0`（`LV_RES_INV`）。

**常见原因：** `USR_CONFIG` littlefs 分区未打包目标资源目录（仓库常不含二进制资源，须按工程脚本打包烧录）。

缺文件通常只影响对应 UI 资源显示，不应影响 WSS/核心控制链路。
