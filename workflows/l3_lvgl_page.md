...
## Design-to-LVGL ?????????2026-07-09?

?????????????????????? UI????? MCP/Agent ???????????? 80% ?????????????????????

### ???????

1. ?????? `design screenshot`?`runtime cut asset`?`reference crop`?`component-only` ???
2. ??????????????????? UI??????????????
3. ????????????????????????????????????????????? LVGL ??????????????
4. ?????????????? alias?????? manifest ????????????????? `ui/` ???????
...
- `analysis_report.json`??? JSON ?????? bbox?????????fallback?
- `debug_overlay.png`????????????????
- ????????????? `debug_loading_template_match.png`?
- `preview.html` ?????????????????????????

??????? `analysis_report.json` ???????????? C ?????????????? `preview.html`?

### ??????

1. ???????????????????????? bbox???????????????
2. ?????????????????????????????????????????/?????????????????????????????????????
3. ?????????????? 70x70 ???????????? template match???????????????????????? JSON ??? `source: button_center_fallback`?
4. ???????????????????????????????????????????????/???????
...
1. `analysis_report.json` ?????????? `template_match`?`bright_circle_detection`?`button_bounds_fallback`?
2. C ???? `lv_obj_set_pos` / `lv_obj_set_size` ???????? bbox??????? `LVGL_LAYOUT_EXCEPTION`?
3. ??????????????? 84x84 ???????? 251x251?40x40 ?????? 70x70 ?????
4. ??????????????????????? `UI_TEXT_*`?`UI_IMG_SRC_*`?`UI_FONT_*`?
5. ????????????????? async post helper???/MQTT/?????????? LVGL API?

### ????

1. ??? quick path?Pillow ???????? `validate_lvgl_layout_code`??? JSON/C ?????
2. `lvgl_render` / `run_lvgl_ui_regression` ?????????????????????? quick path ????????
3. ?????? 2 ?????????????????? artifact?????? 30 ???
4. ?? JPEG ??????????????? runtime path/lazy load??????? C array ???????
