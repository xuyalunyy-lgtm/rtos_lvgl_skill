...
- **工程效率/接手成本**：模块边界不清、队列拓扑缺失 → [module_contract_topology.txt](../prompts/module_contract_topology.txt)（C29-C30）；无界等待、现场不可观测、生命周期不对称 → [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt)（C31-C33）；热路径抖动、关键路径无预算、拷贝过多 → [hotpath_critical_budget.txt](../prompts/hotpath_critical_budget.txt)（C34-C36）；无背压降级、故障不可恢复、配置矩阵散落、无法一键复现、缺回归样本 → [backpressure_recovery_config.txt](../prompts/backpressure_recovery_config.txt)（C37-C41）；板级资源冲突、锁预算缺失/优先级反转、关中断过长、传感器总线/单位/校准契约缺失 → [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt)（C42-C45）
