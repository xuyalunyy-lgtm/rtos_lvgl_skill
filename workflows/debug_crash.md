...
| Hang without Crash / Stop Stuck | **C31.1–C31.4**, C30.4 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) | `blocking_wait_checker.py` |
| Cannot Replay Field Issue / Logs Missing State and Error Codes | **C32.1–C32.5**, C14.7, C14.8 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) · [logging_management_constraints.md](../references/logging_management_constraints.md) | — |
| Crash or Leak after Multiple Stop/Start | **C33.1–C33.5**, C12.4, C24.1 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) · [peripheral_shutdown_safety.txt](../prompts/peripheral_shutdown_safety.txt) | — |
| Periodic Latency Spikes / Callback Stutter | **C34.1–C34.5**, C4.3, C25.4 | [hotpath_critical_budget.txt](../prompts/hotpath_critical_budget.txt) | media checkers + manual |
| Intermittent Timeout at Boot/Network/Audio/Video/UI Stage | **C35.1–C35.5**, C31, C32 | [hotpath_critical_budget.txt](../prompts/hotpath_critical_budget.txt) | stage max time / timeout counter |
| Memory Jitter, Stale Frames, DMA Data Error | **C36.1–C36.5**, C2, C28, C42 | [hotpath_critical_budget.txt](../prompts/hotpath_critical_budget.txt) | media/DMA checkers + manual |
...
| Queue Full Causes Latency Buildup or System Hang | **C37.1–C37.5**, C30, C31 | [backpressure_recovery_config.txt](../prompts/backpressure_recovery_config.txt) | `blocking_wait_checker.py` + watermark counter |
| Only Reboot Recovers after Network/Peripheral Failure | **C38.1–C38.5**, C20, C24, C33 | [backpressure_recovery_config.txt](../prompts/backpressure_recovery_config.txt) | health counter / retry counter |
| Crash Only on Specific Config or Board | **C39.1–C39.5**, C42 | [backpressure_recovery_config.txt](../prompts/backpressure_recovery_config.txt) | profile/config matrix |
| Cannot Reproduce User Field Issue | **C40.1–C40.5**, C14, C41 | [backpressure_recovery_config.txt](../prompts/backpressure_recovery_config.txt) | build/flash/log/decode commands |
