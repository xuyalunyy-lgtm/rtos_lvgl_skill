...
- **平台：** 通用
- **变更：**
  - `scripts/check_installed_runtime.py` — 安装目录审计（文件清单/hash/禁止目录，6 项自测）
  - `scripts/skill_forward_eval.py` — 离线确定性行为回归（5 eval cases，3 项自测）
  - Release manifest — 安装脚本生成 manifest（version/payload hash）
- **验证：**
  - `check_installed_runtime --self-test`：6/6 PASS
  - `skill_forward_eval --self-test`：3/3 PASS（5/5 eval cases）
  - `skill_iterate --release`：PASS
- **版本：** 27.0.8
