...
- **平台：** 通用
- **变更：**
  - `scripts/skill_iterate.py` — 新增 `--release`（16 步完整门禁），`--check` 扩展到 15 步
  - `scripts/check_log_triage_matrix.py` — 严格矩阵（expected_ids/forbidden_ids/expected_counts/exit_code/Windows-safe）
- **验证：** skill_iterate --check 15 步全绿 / check_log_triage_matrix 10/10 / check_codegen_matrix 5/5
- **版本：** 25.0.8
