...
> **v7.0.7 统一框架**：所有 checker 基于 `checker_io.py` 共享基础设施（`read_file`, `make_issue`, `run_checker`），统一支持 `--json` 输出和 `--dir` 递归扫描。
...
| C31 超时预算 | `python tools/blocking_wait_checker.py --dir src/` |
| C36/C37 数据拷贝与背压 | `python tools/efficiency_budget_checker.py --dir src/` |
| C43 锁预算与优先级反转 | `python tools/lock_budget_checker.py --dir src/` |
| C44 临界区/关中断预算 | `python tools/critical_section_checker.py --dir src/` |
| C45 传感器集成契约 | `python tools/sensor_integration_checker.py --dir src/` |
