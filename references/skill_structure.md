...
| `references/release_governance.md` | 20x 提效、大版本重构、提交审计门禁 | 人工 | 同步 |
...
| [l2_architecture_review](../workflows/l2_architecture_review.md) | core_rules + constraint_index | software_architecture_design | 架构同步输出 + check_architecture_sync |
...
| C29 | 模块契约 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C30 | 任务/队列拓扑表 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C31 | 超时预算 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C32 | 可观测性优先 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C33 | 生命周期对称 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C34 | 热路径禁区 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C35 | 关键路径预算表 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C36 | 数据拷贝预算 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C37 | 背压与降级策略 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C38 | 故障隔离与自动恢复 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C39 | 配置矩阵约束 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C40 | 一键复现闭环 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C41 | 回归样本优先 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C42 | 板级资源契约 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C43 | 锁预算与优先级反转防护 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C44 | 临界区/关中断预算 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| C45 | 传感器集成契约 | [runtime_efficiency_contracts.txt](../prompts/runtime_efficiency_contracts.txt) |
| 架构 | 软件架构设计 | [software_architecture_design.txt](../prompts/software_architecture_design.txt) |
...
> C1–C45 是硬性约束域；软件架构设计是场景 prompt，可与 C29-C45 联动但不单独占用约束编号。
...
| 平台 | 文件 | 必选约束 | 特性 |
|------|------|----------|------|
| ESP32 | `esp32.json` | C1-C4,C7-C9,C11-C12,C14-C15,C23,C25-C45 | WiFi+BLE+LVGL+I2S+Camera, 双核, PSRAM |
| STM32 | `stm32.json` | C2-C4,C7-C9,C11-C12,C14-C15,C23,C29-C45 | LVGL+I2S+TLS, 单核 Cortex-M |
| JL | `jl.json` | C1-C4,C6-C15,C23,C25-C45 | WiFi+BLE+LVGL+I2S+语音/视频, 双核 RISC-V |
| BK | `bk.json` | C1-C4,C6-C15,C17,C23,C25-C45 | WiFi+BLE+LVGL+AVDK音频+语音/视频, 双核 IPC |
...
> **v8.0.x 统一框架**：41 个 checker 全量注册到 `checker_registry.py`（`ALL_CHECKERS`），按 suite 分组（default 34 / all 39 / security 5 / media 4 / platform 5 / realtime 5 / enhanced 2）。所有 checker 基于 `checker_io.py` 共享基础设施，统一支持 `--json` / `--dir`。`run_review.py --suite <name>` 按 suite 运行，`--json` 输出结构化报告。
...
| 架构同步审计 | `python scripts/check_architecture_sync.py` |
| Skill 元数据审计 | `python scripts/check_skill_metadata.py` · `python scripts/check_skill_metadata.py --self-test` |
| 提交 / release 审计 | `python scripts/commit_audit.py --self-test` · `python scripts/commit_audit.py --max-log 12 --strict-release` |
...
| 按 suite 运行 checker | `python tools/run_review.py --dir src/ --suite default\|all\|security\|media\|platform\|realtime` |
| 查看 checker 列表 | `python tools/run_review.py --list-checkers` · `--list-checkers --json` |
| 机器可读报告 | `python tools/run_review.py --dir src/ --json` |
| 度量仪表盘 | `python tools/metrics_dashboard.py --project ./my_firmware --suite default` |
| 实时监控 | `python tools/watch_mode.py --dir src/ --suite default` · `--domain C3` |
| 自动修复建议 | `python tools/auto_fix_engine.py file.c --checker cjson` · `--json` |
