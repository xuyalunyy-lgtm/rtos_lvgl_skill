...
```
L0 鎺у埗骞抽潰   SKILL.md              鎰忓浘璺敱 路 閾佸緥绱㈠紩 路 rules锛堢姝㈣啫鑳€锛?
L1 缂栨帓       workflows/*.md        閫夊畾 1 涓?workflow锛屾寜 Step 椤哄簭鎵ц
L2 鎬荤翰       references/           core_rules 路 constraint_index 路 鏈枃浠?
L3 鍦烘櫙       prompts/*.txt         workflow 鎸囧畾 1鈥? 涓紝绂佹鍏ㄥ姞杞?
     骞冲彴     platforms/*.md        workflow Step 1 鍔犺浇 1 涓?
L4 鍙墽琛?    examples/ 路 tools/    瀹屾暣鐗?L2+锛汱ite 鏃犳灞?
```
...
| 宸ュ叿浼樺厛 | `run_review.py` 浠ｆ浛璇?checker 婧愮爜 |
...
鍔犺浇鏂瑰紡锛歚python tools/product_profile.py <platform>` 路 `--json` 路 `--stack <task>`

Agent 鍦?L3 寮€濮嬪墠**鎺ㄨ崘**鍔犺浇浜у搧 profile锛氳嚜鍔ㄨ幏鍙栧繀閫夌害鏉熴€佹爤寤鸿銆佸父瑙佸潙鐐广€?

## 宸ュ叿鐩綍锛堝畬鏁寸増 路 workflow 鍐呰皟鐢級

| 鐢ㄩ€?| 鍛戒护 |
|------|------|
| 涓€閿?L2 | `python tools/run_review.py --dir src/ --platform xxx` |
| Checker 绠＄嚎绱㈠紩 | `python tools/run_review.py --list-checkers` |
| 鑷祴 | `python tools/run_review.py --self-test` |
| 閾佸緥鑼冧緥绾︽潫 | `python tools/run_review.py --validate-examples` |
| 杩愯鏃跺垎鍙戝璁?| `python scripts/check_runtime_distribution.py` |
| Skill 鍏冩暟鎹璁?| `python scripts/check_skill_metadata.py` 路 `python scripts/check_skill_metadata.py --self-test` |
| Lite 鍚屾 | `python scripts/sync_lite.py` 路 Windows锛歚.\scripts\sync_lite.cmd` |
| 杩唬楠岃瘉 | `python scripts/skill_iterate.py --check --sync` 路 Windows锛歚.\scripts\skill_iterate.cmd -Sync` |
| 瀹夎 Cursor | `.\scripts\install_skill.ps1`锛堣 [INSTALL.md](../INSTALL.md)锛?|
| 瀹夎 Codex | `python scripts/install_multi_ide.py --ide codex` |
| 瀹夎 Claude Code | `.\scripts\install_claude_code.ps1`锛堣 [claude_code.md](claude_code.md)锛?|
| C10 璇煶鏃跺簭 | `python tools/voice_sequence_checker.py --dir src/` |
| C18 澶栬椹卞姩 | `python tools/peripheral_driver_checker.py --dir src/` |
| C19 Flash/NVS | `python tools/flash_nvs_checker.py --dir src/` |
| C20 缃戠粶闊ф€?| `python tools/network_resilience_checker.py --dir src/` |
| C21 浣庡姛鑰?| `python tools/low_power_checker.py --dir src/` |
| C23 鏄剧ず椹卞姩 | `python tools/display_driver_checker.py --dir src/` |
| C25 闊宠棰戠绾?| `python tools/av_pipeline_checker.py --dir src/` |
| C26 缂栬В鐮佹牸寮?| `python tools/media_format_checker.py --dir src/` |
| C27 鏃堕挓/Jitter | `python tools/av_clock_jitter_checker.py --dir src/` |
| C28 DMA/cache buffer | `python tools/av_dma_buffer_checker.py --dir src/` |
| C13 鐘舵€佹満 | `python tools/state_machine_checker.py --dir src/` |
| C14.4 鏃ュ織鑴辨晱 | `python tools/log_desensitize_checker.py --dir src/` |
| C16 瀹氭椂鍣?| `python tools/timer_checker.py --dir src/` |
| 閾炬帴妫€鏌?| `python tools/check_links.py` |
| C14 鏃ュ織妫€鏌?| `python tools/logging_checker.py --dir src/` |
| C12 杩斿洖鍊兼鏌?| `python tools/return_check_checker.py --dir src/` |
| C11.5 鍑芥暟闀垮害 | `python tools/function_length_checker.py --dir src/` |
| MVP 楠ㄦ灦 | `python tools/mvp_codegen_tool.py Module --platform jl -o ./generated` |
| 鑷姩绾︽潫鍙戠幇 | `python tools/constraint_discovery.py --dir src/` 路 `--report proposal.md` 路 `--json` |

Checker 涓?C#.# 鏄犲皠 鈫?[examples/README.md](../examples/README.md)
...
鏀瑰満鏅繁缁嗚妭       鈫?prompts/xxx.txt锛堟鏌?workflow 寮曠敤锛?
鏀规楠?杈撳嚭鏍煎紡    鈫?workflows/xxx.md锛堟鏌?SKILL 璺敱琛級
