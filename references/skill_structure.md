...
```
L0 控制平面   SKILL.md              语义路由 · 加载边界 · 交付契约（禁止膨胀）
...
SKILL.md 先按主要交付物选择一个工作流。组合请求按依赖顺序拆成阶段，每个阶段重新路由，避免一次加载多个工作流。

| 模式 | 主要交付物 | 典型 workflow | 核心工具 |
|----|-----------|--------------|---------|
| **review** | 风险、证据与修复建议 | l2_code_review, l2_project_review, l2_memory_analysis, hw_sw_cocodebug | run_review, context_router |
| **debug** | 根因排序、验证探针与修复 | debug_crash | log_triage, context_router |
| **generate** | 可编译的代码、配置或裁剪结果 | l3_lvgl_page, l3_new_module, l3_bring_up, l3_sdk_trim | 目标工程构建与测试工具 |

**铁律：** 当前阶段只加载所选 workflow 直接要求的文件；切换阶段前重新路由。
...
| `project_doctor.py` | 项目入口 — 识别五类 SDK、构建入口与产物，并按自然语言任务生成加载计划 |
...
| 工具 | 职责 |
|------|------|
| `run_review.py` | 一键静态审查（checker 数量以 registry 为准） |
