...
页面规划模板 1.2 在保留 1.1 兼容性的基础上，已将这些责任固化为可校验字段：
...
已有 1.0/1.1 规划文件仍可校验；新建多页工程应使用 1.2 模板。
...
### 页面规划模板 1.2：生产路由、资源预算与固定分辨率例外

1.2 在 1.1 的页面树、显式返回栈和中断恢复契约上，新增了以下必须在设计评审前明确的项目：

- `navigation.production_router_enabled`：必须为 `true`。每一条计划中的路由都必须在量产配置下经过正式路由器；测试宏、循环演示或日志分支不能承载正式跳转。
- `navigation.reset_policy`：限定为 `root_or_recovery`。`reset` 只用于启动、明确的恢复流程或不可恢复的系统中断，不能替代正常的返回路径。
- `layout`：声明响应式或固定参考分辨率、参考尺寸及缩放策略。固定坐标仅在 `fixed_reference` 模式下允许，并必须填写 `fixed_coordinate_rationale`。
- `transition_budget`：给出单次转场的页面创建、同步解码和临时堆分配上限；页面 `resources` 还必须说明图像的解码策略及解码后缓存预算。

#### 路由动作选择矩阵

| 场景 | 路由类型 / 栈操作 | 原因 |
| --- | --- | --- |
| 详情页、设置子页、可恢复的弹窗或告警 | `forward` / `push`；中断为 `interrupt` / `push` | 需要保留调用者及返回动画方向。 |
| 同一内容槽位的 Tab、筛选条件或同级视图 | `forward` / `replace` | 旧视图不能成为“返回”历史，避免无意义的栈增长。 |
| 用户返回、确认后恢复、关闭弹窗 | `back` 或 `return` / `pop` | 从显式栈恢复原页面；无原页面时使用声明的 `fallback_target`。 |
| 启动、恢复出厂后的根入口、不可恢复系统中断 | `reset` / `reset` | 清栈是例外，必须标注 `reset_reason` 为 `boot`、`recovery` 或 `system_interrupt`。 |

已有页面栈并不等于实际拥有返回能力：若量产分支把普通导航统一实现成 `reset_to`，页面缓存和返回链都会被清空，应按设计缺陷处理，而不是把它当作轻量切页优化。

#### 资源与转场预算

- 影响首帧的背景、图标和字体应优先采用 `predecoded`，并为它们声明 `decoded_cache_budget_bytes`；只有在预算不足且已测量时才使用 `lazy_once` 或 `filesystem` 解码。
- 转场路径不得在 UI 线程隐式执行超出 `max_decode_ms` 的同步 PNG/JPEG 解码，也不得突破 `max_heap_alloc_bytes` 的临时分配上限。
- 预算是设备实测约束，而非常量装饰：在目标硬件上记录页面创建耗时、同步解码耗时、峰值堆变化和转场后的首帧时间，并在性能回归时复测。

#### 固定参考分辨率的边界

固定坐标适用于尺寸、安装方向和像素密度都已锁定的专用设备；它不是跳过布局设计的理由。采用它时，规划中必须同时写明参考分辨率、缩放方式、字体回退或截断策略、图片解码缓存上限和显存/SRAM 预算。需要适配多分辨率、多语言或动态文本长度时，优先使用 Flex/Grid 或约束式布局。

新建多页项目应使用 1.2 模板；1.0/1.1 文件仍由校验器兼容读取。导航报告会同时展示生产路由声明、布局和转场/资源预算，作为评审附件。
