...
| ID | 约束 | 严重度 | 验证 | 正例 | 反例 |
|----|------|--------|------|------|------|
| C28.1 | Camera/I2S/LCD/codec DMA buffer 必须位于 DMA-capable 内存，并满足 cache line 或 DMA 控制器对齐；禁止普通 `malloc`/`pvPortMalloc` 作为媒体 DMA buffer | P0 | `av_dma_buffer_checker.py` + 人工 | [good_av_dma_buffer_lifecycle.c](../examples/good_av_dma_buffer_lifecycle.c) | [bad_av_dma_buffer_lifecycle.c](../examples/bad_av_dma_buffer_lifecycle.c) |
