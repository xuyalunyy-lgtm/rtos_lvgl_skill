...
| 接手模块需反复追调用上下文 / 所有权 | **C29.1–C29.5** + [module_contract_topology.txt](../prompts/module_contract_topology.txt) |
...
| C29.2 | 模块 API 必须声明阻塞语义、最大等待时间、是否可重入；默认不得假设调用者知道内部等待点 | P0 | 人工 + `blocking_wait_checker.py` | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | `module_read()` 内部无限等 Queue |
| C29.3 | 模块必须声明入参、出参、Queue payload、callback user_data 的所有权与释放方 | P1 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | callback 传裸指针但未说明生命周期 |
| C29.4 | 模块必须声明 `init/start/stop/deinit` 合法顺序、重复调用行为和半初始化清理策略 | P1 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | `stop()` 未说明 init 失败后能否调用 |
| C29.5 | 模块必须声明错误码语义：timeout/resource/config/bug 等分类，以及哪些错误可恢复 | P2 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | 全部返回 `-1` |
...
| 模块越来越像上帝对象，改网络牵动 UI/存储/驱动 | C29.6-C29.10 模块边界缺失 |
...
| C30.2 | 每条 Queue 必须声明元素类型、payload 所有权、生产者和消费者；跨核/跨线程路径须注明边界 | P0 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | 只看到 `xQueueCreate(8, sizeof(void*))` [QUEUE_CREATE] |
| C30.3 | 每条 Queue 必须声明深度、发送/接收 timeout 和满队列策略：drop-oldest/drop-newest/backpressure/overwrite | P1 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | 满队列时无限等 |
| C30.4 | 每个 task 必须有退出条件、stop flag 或通知路径；禁止只能通过 reboot 终止 | P1 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | while(1) 无退出和状态 |
| C30.5 | 拓扑表必须保留 queue high-water、drop、timeout 或 overflow 观测点 | P2 | 人工 | [module_contract_topology.txt](../prompts/module_contract_topology.txt) | 队列满无法复盘 |
...
| C32.2 | 关键链路必须计数 timeout/drop/retry/reconnect/overflow/underrun 等现场指标 | P1 | 人工 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) | 只有平均成功率 |
| C32.3 | 任务必须可采集 stack high-water、queue high-water、heap free/min/largest 等资源水位 | P1 | 人工 + `stack_calculator.py` | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) | 只看总 free heap |
| C32.4 | init/connect/handshake/decode/render/flush 等阶段必须保留 max time 或最近耗时；LVGL 路径可声明外部遥测 owner | P2 | `observability_checker.py` + 人工 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) | 性能尖峰不可定位 |
...
| C33.2 | `alloc/create/register/attach/subscribe` 必须有对应 `free/delete/unregister/detach/unsubscribe` | P0 | 人工 | [timeout_lifecycle_observability.txt](../prompts/timeout_lifecycle_observability.txt) | 注册回调后 never unregister |
...
| C39.6 | Zephyr 应用 C/H 源文件不得 `#define` / `#undef CONFIG_*`；符号须归属 Kconfig/prj.conf | P1 | `zephyr_pattern_checker.py` | `CONFIG_FOO=y` in `prj.conf` | `#define CONFIG_FOO y` |
