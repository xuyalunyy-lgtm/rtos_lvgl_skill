...
> 45 个约束域、248 条规则之间的关系网络。修改一条约束时，参考本图评估连锁影响。
...
| C19 → C22 | Flash/NVS 分区管理（C19）是 OTA 分区表（C22.3）和回滚（C22.2）的基础 |
| C9 → C22 | 密钥安全（C9）是 OTA 签名验证（C22.1）的密钥管理基础 |
| C22 → C20 | OTA 升级后的重连（C22）依赖网络韧性（C20）的退避/降级策略 |
| C22 → C31 | OTA HTTP 下载（C22.5）必须遵循超时预算（C31） |
...
| **C22.2** OTA mark_valid | **C38.2** 故障恢复 | 新固件自检失败 vs 自动回滚 | 首次启动做 health check，失败则不 mark_valid，让 bootloader 自动回滚 |
| **C22.5** OTA 超时重试 | **C35.1** 关键路径预算 | OTA 耗时 vs 启动预算 | OTA 设独立超时预算，不影响 boot 主路径；失败后离线模式 |
...
| 改 C22（OTA 安全） | C19（Flash/NVS）、C9（密钥）、C20（网络韧性）、C31（超时）、C38（故障恢复） | 分区表 + 签名验证 + 重连 + HTTP 超时 + 回滚策略 |
