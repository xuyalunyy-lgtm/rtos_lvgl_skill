...
- 32.0.2: ESP32 平台文档已覆盖 OTA/heap_caps/PSRAM/DMA/WDT/NVS/WiFi power-save
- 32.0.3: Zephyr 文档新增 Workqueue + MCUboot/OTA 章节
- 32.0.4: 平台感知 checker 通过 sdk_lookup 统一识别 API
- 32.0.5: 新增 rtos_scanner.py 轻量拓扑扫描器（只报告，不阻断 gate）
...
- **checker 修复**：`priority_checker` 区分 SEM_CREATE/MUTEX_CREATE，消除 C15.2 误报；`ota_safety_checker` 明确 C22.1 验证证据（secure boot/signed OTA/esp_image_verify）
- **SDK 抽象层稳定**：修复 `get_all_apis_by_category()` 类别查询；平台映射 unsupported 标记；validate-constraints 噪音过滤
- **路由收敛**：SKILL.md 默认路由只暴露用户主路径（review/crash/memory/bring-up/LVGL），治理内容降级为 maintainer-only
- **agents/openai.yaml**：default_prompt 移除 release-audit，聚焦 FreeRTOS/ESP32/Zephyr + 约束 + checker + crash

## 29.0.0 — 2026-07-02

V29：SDK 抽象中间件层 — 平台无关的 API 查询引擎。

- `references/sdk_abstraction.yaml`：68 个标准操作注册表
- `platforms/{esp32,stm32,jl,bk,zephyr}_sdk_map.yaml`：5 平台映射
- `tools/sdk_lookup.py`：查询引擎
- 27 个 checker 改为通过 sdk_lookup 获取 API 列表
- constraint_detail.md/core_rules.md 标注标准操作名 [OP_NAME]
